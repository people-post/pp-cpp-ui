...
#include <string.h>
	...
	#define UI_SHADER_HEADER_VERSION PP_BROWSER_SHADER_HEADER_GLES
	#if defined(__APPLE__) && TARGET_OS_IPHONE
		#include <OpenGLES/ES3/gl.h>
	#else
		#include <GLES3/gl3.h>
	#endif
#elif defined UI_GL3_CUSTOM_LOADER
...
#ifndef UI_NUM_MSAA_SAMPLES
	#if defined(UI_GL_ES3)
		// Multisampled FBOs are unreliable on some GLES drivers (notably iOS Simulator).
		#define UI_NUM_MSAA_SAMPLES 0
	#else
		#define UI_NUM_MSAA_SAMPLES 2
	#endif
#endif
...
#define UI_STRINGIFY_IMPL(x) #x
#define UI_STRINGIFY(x) UI_STRINGIFY_IMPL(x)

#define UI_SHADER_HEADER \
	UI_SHADER_HEADER_VERSION "#define MAX_NUM_STOPS " UI_STRINGIFY(MAX_NUM_STOPS) "\n#line " UI_STRINGIFY(__LINE__) "\n"

static const char* shader_vert_main = UI_SHADER_HEADER R"(
...
static const char* shader_frag_texture = UI_SHADER_HEADER R"(
...
static const char* shader_frag_color = UI_SHADER_HEADER R"(
in vec2 fragTexCoord;
in vec4 fragColor;
...
static const char* shader_frag_gradient = UI_SHADER_HEADER R"(
#define LINEAR 0
#define RADIAL 1
#define CONIC 2
...
static const char* shader_frag_creation = UI_SHADER_HEADER R"(
...
static const char* shader_vert_passthrough = UI_SHADER_HEADER R"(
in vec2 inPosition;
in vec2 inTexCoord0;
...
static const char* shader_frag_passthrough = UI_SHADER_HEADER R"(
uniform sampler2D _tex;
in vec2 fragTexCoord;
out vec4 finalColor;
...
static const char* shader_frag_color_matrix = UI_SHADER_HEADER R"(
uniform sampler2D _tex;
...
static const char* shader_frag_blend_mask = UI_SHADER_HEADER R"(
uniform sampler2D _tex;
...
#define UI_SHADER_BLUR_HEADER \
	UI_SHADER_HEADER "\n#define BLUR_SIZE " UI_STRINGIFY(BLUR_SIZE) "\n#define BLUR_NUM_WEIGHTS " UI_STRINGIFY(BLUR_NUM_WEIGHTS)

static const char* shader_vert_blur = UI_SHADER_BLUR_HEADER R"(
...
static const char* shader_frag_blur = UI_SHADER_BLUR_HEADER R"(
uniform sampler2D _tex;
...
static const char* shader_frag_drop_shadow = UI_SHADER_HEADER R"(
uniform sampler2D _tex;
uniform vec2 _texCoordMin;
uniform vec2 _texCoordMax;
uniform vec4 _color;
...
public:
	...
	{
		UI_ASSERT((size_t)id < (size_t)Enum::Count);
...
private:
	ui::Array<T, (size_t)Enum::Count> ids = {};
};
...
static void CheckGLError(const char* operation_name)
{
	...
	{
		static const ui::Pair<GLenum, const char*> error_names[] = {{GL_INVALID_ENUM, "GL_INVALID_ENUM"}, {GL_INVALID_VALUE, "GL_INVALID_VALUE"},
		...
		ui::Log::Message(ui::Log::LT_ERROR, "OpenGL error during %s. Error code 0x%x (%s).", operation_name, error_code, error_str);
	}
#endif
	(void)operation_name;
}
...
{
	UI_ASSERT(shader_type == GL_VERTEX_SHADER || shader_type == GL_FRAGMENT_SHADER);
	...
	if (status == GL_FALSE)
	{
		...
		ui::Log::Message(ui::Log::LT_ERROR, "Compile failure in OpenGL shader: %s", info_log_string);
...
static bool CreateProgram(GLuint& out_program, Uniforms& inout_uniform_map, ProgramId program_id, GLuint vertex_shader, GLuint fragment_shader)
{
	...
	if (status == GL_FALSE)
	{
		...
		ui::Log::Message(ui::Log::LT_ERROR, "OpenGL program linking failure: %s", info_log_string);
		delete[] info_log_string;
		glDeleteProgram(id);
		return false;
	}
	...
	for (int unif = 0; unif < num_active_uniforms; ++unif)
	{
		...
		{
			ui::Log::Message(ui::Log::LT_ERROR, "OpenGL program uses unknown uniform '%s'.", name_buf);
			return false;
		}
	}
	...
	GLuint shared_depth_stencil_buffer)
{
	...
	const ui::Colourf border_color(0.f, 0.f);
	...
	{
		ui::Log::Message(ui::Log::LT_ERROR, "OpenGL framebuffer could not be generated. Error code %x.", framebuffer_status);
		return false;
	}
...
static GLuint CreateTexture(ui::Span<const ui::byte> source_data, ui::Vector2i source_dimensions)
{
	GLuint texture_id = 0;
...
static void BindTexture(const FramebufferData& fb)
{
	if (!fb.color_tex_buffer)
	{
		UI_ERRORMSG("Only framebuffers with color textures can be bound as textures. This framebuffer probably uses multisampling which needs a "
					   "blit step first.");
	}
...
{
	UI_ASSERT(std::all_of(data.vert_shaders.begin(), data.vert_shaders.end(), [](auto&& value) { return value == 0; }));
	UI_ASSERT(std::all_of(data.frag_shaders.begin(), data.frag_shaders.end(), [](auto&& value) { return value == 0; }));
	UI_ASSERT(std::all_of(data.programs.begin(), data.programs.end(), [](auto&& value) { return value == 0; }));
	auto ReportError = [](const char* type, const char* name) {
		ui::Log::Message(ui::Log::LT_ERROR, "Could not create OpenGL %s: '%s'.", type, name);
		return false;
	};
...
{
	auto mut_program_data = ui::MakeUnique<Gfx::ProgramData>();
	if (Gfx::CreateShaders(*mut_program_data))
	{
		program_data = std::move(mut_program_data);
		ui::Mesh mesh;
		ui::MeshUtilities::GenerateQuad(mesh, ui::Vector2f(-1), ui::Vector2f(2), {});
...
void RenderInterface_GL3::RecoverGpuResources()
{
	...
	if (!Gfx::CreateShaders(*mut_program_data))
	{
		ui::Log::Message(ui::Log::LT_ERROR, "Failed to recreate OpenGL shaders after device reset.");
		return;
	}
...
{
	viewport_width = ui::Math::Max(width, 1);
	viewport_height = ui::Math::Max(height, 1);
	viewport_offset_x = offset_x;
	viewport_offset_y = offset_y;
	projection = ui::Matrix4f::ProjectOrtho(0, (float)viewport_width, (float)viewport_height, 0, -10000, 10000);
}

void RenderInterface_GL3::BeginFrame()
{
	UI_ASSERT(viewport_width >= 1 && viewport_height >= 1);
	...
	scissor_state = ui::Rectanglei::MakeInvalid();
...
ui::CompiledGeometryHandle RenderInterface_GL3::CompileGeometry(ui::Span<const ui::Vertex> vertices, ui::Span<const int> indices)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(ui::Vertex) * vertices.size(), (const void*)vertices.data(), draw_usage);

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::Position);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Position, 2, GL_FLOAT, GL_FALSE, sizeof(ui::Vertex),
		(const GLvoid*)(offsetof(ui::Vertex, position)));

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::Color0);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Color0, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ui::Vertex),
		(const GLvoid*)(offsetof(ui::Vertex, colour)));

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::TexCoord0);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::TexCoord0, 2, GL_FLOAT, GL_FALSE, sizeof(ui::Vertex),
		(const GLvoid*)(offsetof(ui::Vertex, tex_coord)));
	...
	return (ui::CompiledGeometryHandle)geometry;
}

void RenderInterface_GL3::RenderGeometry(ui::CompiledGeometryHandle handle, ui::Vector2f translation, ui::TextureHandle texture)
{
...
void RenderInterface_GL3::ReleaseGeometry(ui::CompiledGeometryHandle handle)
{
	Gfx::CompiledGeometryData* geometry = (Gfx::CompiledGeometryData*)handle;
...
/// @note Changes the coordinate system from pp-cpp-ui to OpenGL, or equivalently in reverse.
/// @note The Rectangle::Top and Rectangle::Bottom members will have reverse meaning in the returned rectangle.
static ui::Rectanglei VerticallyFlipped(ui::Rectanglei rect, int viewport_height)
{
	UI_ASSERT(rect.Valid());
	ui::Rectanglei flipped_rect = rect;
	flipped_rect.p0.y = viewport_height - rect.p1.y;
	flipped_rect.p1.y = viewport_height - rect.p0.y;
	return flipped_rect;
}

void RenderInterface_GL3::SetScissor(ui::Rectanglei region, bool vertically_flip)
{
	...
	if (region.Valid() && region != scissor_state)
	{
		...
		const int x = ui::Math::Clamp(region.Left(), 0, viewport_width);
		const int y = ui::Math::Clamp(viewport_height - region.Bottom(), 0, viewport_height);
...
void RenderInterface_GL3::EnableScissorRegion(bool enable)
{
	...
	if (!enable)
		SetScissor(ui::Rectanglei::MakeInvalid(), false);
}

void RenderInterface_GL3::SetScissorRegion(ui::Rectanglei region)
{
	SetScissor(region);
}
...
void RenderInterface_GL3::RenderToClipMask(ui::ClipMaskOperation operation, ui::CompiledGeometryHandle geometry, ui::Vector2f translation)
{
	UI_ASSERT(glIsEnabled(GL_STENCIL_TEST));
	using ui::ClipMaskOperation;
...
ui::TextureHandle RenderInterface_GL3::LoadTexture(ui::Vector2i& texture_dimensions, const ui::String& source)
{
	...
	{
		ui::Log::Message(ui::Log::LT_ERROR, "Texture file size is smaller than TGAHeader, file is not a valid TGA image.");
	...
	ui::UniquePtr<byte[]> image_dest_buffer(new byte[image_size]);
...
ui::TextureHandle RenderInterface_GL3::GenerateTexture(ui::Span<const ui::byte> source_data, ui::Vector2i source_dimensions)
{
	UI_ASSERT(source_data.data() && source_data.size() == size_t(source_dimensions.x * source_dimensions.y * 4));

	GLuint texture_id = Gfx::CreateTexture(source_data, source_dimensions);
	if (texture_id == 0)
	{
		ui::Log::Message(ui::Log::LT_ERROR, "Failed to generate texture.");
		return {};
	}
	return (ui::TextureHandle)texture_id;
}
...
void RenderInterface_GL3::DrawFullscreenQuad(ui::Vector2f uv_offset, ui::Vector2f uv_scaling)
{
	ui::Mesh mesh;
	ui::MeshUtilities::GenerateQuad(mesh, ui::Vector2f(-1), ui::Vector2f(2), {});
	if (uv_offset != ui::Vector2f() || uv_scaling != ui::Vector2f(1.f))
	{
		for (ui::Vertex& vertex : mesh.vertices)
			vertex.tex_coord = (vertex.tex_coord * uv_scaling) + uv_offset;
	}
	const ui::CompiledGeometryHandle geometry = CompileGeometry(mesh.vertices, mesh.indices);
	RenderGeometry(geometry, {}, RenderInterface_GL3::TexturePostprocess);
	ReleaseGeometry(geometry);
}

static ui::Colourf ConvertToColorf(ui::ColourbPremultiplied c0)
{
	ui::Colourf result;
	for (int i = 0; i < 4; i++)
...
static void SigmaToParameters(const float desired_sigma, int& out_pass_level, float& out_sigma)
{
	...
	out_pass_level = ui::Math::Clamp(ui::Math::Log2(int(desired_sigma * (2.f / max_single_pass_sigma))), 0, max_num_passes);
	out_sigma = ui::Math::Clamp(desired_sigma / float(1 << out_pass_level), 0.0f, max_single_pass_sigma);
}

static void SetTexCoordLimits(GLint tex_coord_min_location, GLint tex_coord_max_location, ui::Rectanglei rectangle_flipped,
	ui::Vector2i framebuffer_size)
{
	...
	const ui::Vector2f min = (ui::Vector2f(rectangle_flipped.p0) + ui::Vector2f(0.5f)) / ui::Vector2f(framebuffer_size);
	const ui::Vector2f max = (ui::Vector2f(rectangle_flipped.p1) - ui::Vector2f(0.5f)) / ui::Vector2f(framebuffer_size);
...
static void SetBlurWeights(GLint weights_location, float sigma)
{
	...
	{
		if (ui::Math::Absolute(sigma) < 0.1f)
			weights[i] = float(i == 0);
		else
			weights[i] = ui::Math::Exp(-float(i * i) / (2.0f * sigma * sigma)) / (ui::Math::SquareRoot(2.f * ui::Math::UI_PI) * sigma);
...
void RenderInterface_GL3::RenderBlur(float sigma, const Gfx::FramebufferData& source_destination, const Gfx::FramebufferData& temp,
	const ui::Rectanglei window_flipped)
{
	UI_ASSERT(&source_destination != &temp && source_destination.width == temp.width && source_destination.height == temp.height);
	UI_ASSERT(window_flipped.Valid());

	int pass_level = 0;
	SigmaToParameters(sigma, pass_level, sigma);

	const ui::Rectanglei original_scissor = scissor_state;

	// Begin by downscaling so that the blur pass can be done at a reduced resolution for large sigma.
	ui::Rectanglei scissor = window_flipped;
	...
	// neighboring texels.
	const ui::Vector2f uv_scaling = {(source_destination.width % 2 == 1) ? (1.f - 1.f / float(source_destination.width)) : 1.f,
		(source_destination.height % 2 == 1) ? (1.f - 1.f / float(source_destination.height)) : 1.f};

	for (int i = 0; i < pass_level; i++)
	{
		scissor.p0 = (scissor.p0 + ui::Vector2i(1)) / 2;
		scissor.p1 = ui::Math::Max(scissor.p1 / 2, scissor.p0);
	...
	auto SetTexelOffset = [texel_offset_location](ui::Vector2f blur_direction, int texture_dimension) {
		const ui::Vector2f texel_offset = blur_direction * (1.0f / float(texture_dimension));
	...
	const ui::Vector2i src_min = scissor.p0;
	const ui::Vector2i src_max = scissor.p1;
	const ui::Vector2i dst_min = window_flipped.p0;
	const ui::Vector2i dst_max = window_flipped.p1;
	...
	const ui::Vector2i target_min = src_min * (1 << pass_level);
	const ui::Vector2i target_max = src_max * (1 << pass_level);
...
void RenderInterface_GL3::ReleaseTexture(ui::TextureHandle texture_handle)
{
	glDeleteTextures(1, (GLuint*)&texture_handle);
}

void RenderInterface_GL3::SetTransform(const ui::Matrix4f* new_transform)
{
...
struct CompiledFilter {
	...
	// Drop shadow
	ui::Vector2f offset;
	ui::ColourbPremultiplied color;

	// ColorMatrix
	ui::Matrix4f color_matrix;
};

ui::CompiledFilterHandle RenderInterface_GL3::CompileFilter(const ui::String& name, const ui::Dictionary& parameters)
{
	...
	if (name == "opacity")
	{
		...
		filter.blend_factor = ui::Get(parameters, "value", 1.0f);
	}
	else if (name == "blur")
	{
		filter.type = FilterType::Blur;
		filter.sigma = ui::Get(parameters, "sigma", 1.0f);
	}
	else if (name == "drop-shadow")
	{
		filter.type = FilterType::DropShadow;
		filter.sigma = ui::Get(parameters, "sigma", 0.f);
		filter.color = ui::Get(parameters, "color", ui::Colourb()).ToPremultiplied();
		filter.offset = ui::Get(parameters, "offset", ui::Vector2f(0.f));
	}
	else if (name == "brightness")
	{
		filter.type = FilterType::ColorMatrix;
		const float value = ui::Get(parameters, "value", 1.0f);
		filter.color_matrix = ui::Matrix4f::Diag(value, value, value, 1.f);
	}
	else if (name == "contrast")
	{
		...
		filter.color_matrix = ui::Matrix4f::Diag(value, value, value, 1.f);
		filter.color_matrix.SetColumn(3, ui::Vector4f(grayness, grayness, grayness, 1.f));
	}
	else if (name == "invert")
	{
		filter.type = FilterType::ColorMatrix;
		const float value = ui::Math::Clamp(ui::Get(parameters, "value", 1.0f), 0.f, 1.f);
		const float inverted = 1.f - 2.f * value;
		filter.color_matrix = ui::Matrix4f::Diag(inverted, inverted, inverted, 1.f);
		filter.color_matrix.SetColumn(3, ui::Vector4f(value, value, value, 1.f));
	}
	else if (name == "grayscale")
	{
		...
		const ui::Vector3f gray = value * ui::Vector3f(0.2126f, 0.7152f, 0.0722f);
		// clang-format off
		filter.color_matrix = ui::Matrix4f::FromRows(
	...
	else if (name == "sepia")
	{
		...
		const float value = ui::Get(parameters, "value", 1.0f);
		const float rev_value = 1.f - value;
		const ui::Vector3f r_mix = value * ui::Vector3f(0.393f, 0.769f, 0.189f);
		const ui::Vector3f g_mix = value * ui::Vector3f(0.349f, 0.686f, 0.168f);
		const ui::Vector3f b_mix = value * ui::Vector3f(0.272f, 0.534f, 0.131f);
		// clang-format off
		filter.color_matrix = ui::Matrix4f::FromRows(
	...
	else if (name == "hue-rotate")
	{
		...
		filter.type = FilterType::ColorMatrix;
		const float value = ui::Get(parameters, "value", 1.0f);
		const float s = ui::Math::Sin(value);
		const float c = ui::Math::Cos(value);
		// clang-format off
		filter.color_matrix = ui::Matrix4f::FromRows(
	...
	if (filter.type != FilterType::Invalid)
		return reinterpret_cast<ui::CompiledFilterHandle>(new CompiledFilter(std::move(filter)));

	ui::Log::Message(ui::Log::LT_WARNING, "Unsupported filter type '%s'.", name.c_str());
	return {};
}

void RenderInterface_GL3::ReleaseFilter(ui::CompiledFilterHandle filter)
{
...
struct CompiledShader {
	...
	ui::Vector2f p;
	ui::Vector2f v;
	ui::Vector<float> stop_positions;
	ui::Vector<ui::Colourf> stop_colors;

	// Shader
	ui::Vector2f dimensions;
};

ui::CompiledShaderHandle RenderInterface_GL3::CompileShader(const ui::String& name, const ui::Dictionary& parameters)
{
	auto ApplyColorStopList = [](CompiledShader& shader, const ui::Dictionary& shader_parameters) {
		auto it = shader_parameters.find("color_stop_list");
		UI_ASSERT(it != shader_parameters.end() && it->second.GetType() == ui::Variant::COLORSTOPLIST);
		const ui::ColorStopList& color_stop_list = it->second.GetReference<ui::ColorStopList>();
		const int num_stops = ui::Math::Min((int)color_stop_list.size(), MAX_NUM_STOPS);

		shader.stop_positions.resize(num_stops);
		shader.stop_colors.resize(num_stops);
		for (int i = 0; i < num_stops; i++)
		{
			const ui::ColorStop& stop = color_stop_list[i];
			UI_ASSERT(stop.position.unit == ui::Unit::NUMBER);
	...
	if (name == "linear-gradient")
	{
		...
		const bool repeating = ui::Get(parameters, "repeating", false);
		shader.gradient_function = (repeating ? ShaderGradientFunction::RepeatingLinear : ShaderGradientFunction::Linear);
		shader.p = ui::Get(parameters, "p0", ui::Vector2f(0.f));
		shader.v = ui::Get(parameters, "p1", ui::Vector2f(0.f)) - shader.p;
	...
	else if (name == "radial-gradient")
	{
		...
		shader.p = ui::Get(parameters, "center", ui::Vector2f(0.f));
		shader.v = ui::Vector2f(1.f) / ui::Get(parameters, "radius", ui::Vector2f(1.f));
		ApplyColorStopList(shader, parameters);
	}
	else if (name == "conic-gradient")
	{
		...
		shader.p = ui::Get(parameters, "center", ui::Vector2f(0.f));
		const float angle = ui::Get(parameters, "angle", 0.f);
		shader.v = {ui::Math::Cos(angle), ui::Math::Sin(angle)};
		ApplyColorStopList(shader, parameters);
	}
	else if (name == "shader")
	{
		const ui::String value = ui::Get(parameters, "value", ui::String());
		if (value == "creation")
		{
			shader.type = CompiledShaderType::Creation;
			shader.dimensions = ui::Get(parameters, "dimensions", ui::Vector2f(0.f));
		}
	}

	if (shader.type != CompiledShaderType::Invalid)
		return reinterpret_cast<ui::CompiledShaderHandle>(new CompiledShader(std::move(shader)));

	ui::Log::Message(ui::Log::LT_WARNING, "Unsupported shader type '%s'.", name.c_str());
	return {};
}

void RenderInterface_GL3::RenderShader(ui::CompiledShaderHandle shader_handle, ui::CompiledGeometryHandle geometry_handle,
	ui::Vector2f translation, ui::TextureHandle /*texture*/)
{
	UI_ASSERT(shader_handle && geometry_handle);
	...
	{
		UI_ASSERT(shader.stop_positions.size() == shader.stop_colors.size());
	...
	{
		const double time = ui::GetSystemInterface()->GetElapsedTime();
	...
	{
		ui::Log::Message(ui::Log::LT_WARNING, "Unhandled render shader %d.", (int)type);
	}
	break;
	}
...
void RenderInterface_GL3::ReleaseShader(ui::CompiledShaderHandle shader_handle)
{
...
void RenderInterface_GL3::BlitTopLayerRegion(ui::Rectanglei src_region_top_left, unsigned int dest_framebuffer, int dest_width,
	int dest_height)
{
	if (!src_region_top_left.Valid() || dest_width <= 0 || dest_height <= 0)
		return;

	ui::Rectanglei clamped = src_region_top_left;
	clamped.p0.x = ui::Math::Clamp(clamped.p0.x, 0, viewport_width);
	clamped.p0.y = ui::Math::Clamp(clamped.p0.y, 0, viewport_height);
	clamped.p1.x = ui::Math::Clamp(clamped.p1.x, 0, viewport_width);
	clamped.p1.y = ui::Math::Clamp(clamped.p1.y, 0, viewport_height);
	if (!clamped.Valid())
		return;

	const ui::Rectanglei src_flipped = VerticallyFlipped(clamped, viewport_height);

	// Scissor restricts glBlitFramebuffer source/dest; UI clipping is usually still active here.
	const ui::Rectanglei saved_scissor = scissor_state;
...
void RenderInterface_GL3::BlitLayerToPostprocessPrimary(ui::LayerHandle layer_handle)
{
...
void RenderInterface_GL3::RenderFilters(ui::Span<const ui::CompiledFilterHandle> filter_handles)
{
	for (const ui::CompiledFilterHandle filter_handle : filter_handles)
	{
		...
		case FilterType::Blur:
		{
			...
			const ui::Rectanglei window_flipped = VerticallyFlipped(scissor_state, viewport_height);
		...
		case FilterType::DropShadow:
		{
			...
			ui::Colourf color = ConvertToColorf(filter.color);
			...
			const ui::Vector2f uv_offset = filter.offset / ui::Vector2f(-(float)viewport_width, (float)viewport_height);
		...
		case FilterType::ColorMatrix:
		{
			...
			constexpr bool transpose = std::is_same<decltype(filter.color_matrix), ui::RowMajorMatrix4f>::value;
		...
		{
			ui::Log::Message(ui::Log::LT_WARNING, "Unhandled render filter %d.", (int)type);
		}
		break;
		}
	}
...
ui::LayerHandle RenderInterface_GL3::PushLayer()
{
	const ui::LayerHandle layer_handle = render_layers.PushLayer();
...
void RenderInterface_GL3::CompositeLayers(ui::LayerHandle source_handle, ui::LayerHandle destination_handle, ui::BlendMode blend_mode,
	ui::Span<const ui::CompiledFilterHandle> filters)
{
	using ui::BlendMode;
...
ui::TextureHandle RenderInterface_GL3::SaveLayerAsTexture()
{
	UI_ASSERT(scissor_state.Valid());
	const ui::Rectanglei bounds = scissor_state;

	GLuint render_texture = Gfx::CreateTexture({}, bounds.Size());
	if (render_texture == 0)
	{
		ui::Log::Message(ui::Log::LT_ERROR, "Failed to create render texture.");
		return {};
	}
	...
	return (ui::TextureHandle)render_texture;
}

ui::CompiledFilterHandle RenderInterface_GL3::SaveLayerAsMaskImage()
{
	BlitLayerToPostprocessPrimary(render_layers.GetTopLayerHandle());
...
void RenderInterface_GL3::SubmitTransformUniform(ui::Vector2f translation)
{
...
ui::LayerHandle RenderInterface_GL3::RenderLayerStack::PushLayer()
{
	UI_ASSERT(layers_size <= (int)fb_layers.size());
	...
	if (layers_size == (int)fb_layers.size())
	{
		...
		Gfx::CreateFramebuffer(fb_layers.back(), width, height, UI_NUM_MSAA_SAMPLES, Gfx::FramebufferAttachment::DepthStencil,
...
{
	UI_ASSERT(layers_size > 0);
	layers_size -= 1;
}

const Gfx::FramebufferData& RenderInterface_GL3::RenderLayerStack::GetLayer(ui::LayerHandle layer) const
{
	UI_ASSERT((size_t)layer < (size_t)layers_size);
...
ui::LayerHandle RenderInterface_GL3::RenderLayerStack::GetTopLayerHandle() const
{
	UI_ASSERT(layers_size > 0);
	return static_cast<ui::LayerHandle>(layers_size - 1);
}
...
{
	UI_ASSERT(layers_size == 1);
	PopLayer();
}

void RenderInterface_GL3::RenderLayerStack::DestroyFramebuffers()
{
	UI_ASSERTMSG(layers_size == 0, "Do not call this during frame rendering, that is, between BeginFrame() and EndFrame().");
...
{
	UI_ASSERT(index < (int)fb_postprocess.size())
...
const ui::Matrix4f& RenderInterface_GL3::GetTransform() const
{
	return transform;
}
...
bool RmlGL3::Initialize(ui::String* out_message)
{
#if defined(UI_GL_ES3)
	if (out_message)
	...
	if (out_message)
		*out_message = ui::CreateString("Loaded OpenGL %d.%d.", GLAD_VERSION_MAJOR(gl_version), GLAD_VERSION_MINOR(gl_version));
#endif
...
{
#if !defined(UI_GL_ES3) && !defined(UI_GL3_CUSTOM_LOADER)
	gladLoaderUnloadGL();
#endif
}
