...
/**
    ...
    By default, pp-cpp-ui will use its own font engine with characters rendered through FreeType. To use your own engine,
    provide a concrete implementation of this class and install it through ui::SetFontEngineInterface().
 */

class UI_CORE_API FontEngineInterface {
public:
	FontEngineInterface();
	...
	/// Called by pp-cpp-ui when it wants to load a font face from file.
	...
	/// Called by pp-cpp-ui when it wants to load a font face from memory, registered using the provided family, style, and weight.
	...
	/// Called by pp-cpp-ui when a font configuration is resolved for an element. Should return a handle that
	...
	/// Called by pp-cpp-ui when a list of font effects is resolved for an element with a given font face.
	...
	/// Called by pp-cpp-ui when it wants to retrieve the width of a string when rendered with this handle.
	/// @param[in] handle The font handle.
	...
	/// Called by pp-cpp-ui when it wants to retrieve the meshes required to render a single line of text.
	...
	/// Called by pp-cpp-ui to determine if the text geometry is required to be re-generated. Whenever the returned version
	...
	/// Called by pp-cpp-ui when it wants to garbage collect memory used by fonts.
