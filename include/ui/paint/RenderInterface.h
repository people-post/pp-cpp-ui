...
/**
    ...
    implementation of this class and install it through ui::SetRenderInterface() in order for anything to be rendered.
 */

class UI_CORE_API RenderInterface : public NonCopyMoveable {
public:
	RenderInterface();
	...
	/// Called by pp-cpp-ui when it wants to compile geometry to be rendered later.
	...
	/// Called by pp-cpp-ui when it wants to render geometry.
	...
	/// Called by pp-cpp-ui when it wants to release geometry.
	/// @param[in] geometry The geometry to release.
	virtual void ReleaseGeometry(CompiledGeometryHandle geometry) = 0;

	/// Called by pp-cpp-ui when a texture is required by the library.
	...
	/// Called by pp-cpp-ui when a texture is required to be generated from a sequence of pixels in memory.
	...
	/// Called by pp-cpp-ui when a loaded or generated texture is no longer required.
	/// @param[in] texture The texture handle to release.
	virtual void ReleaseTexture(TextureHandle texture) = 0;

	/// Called by pp-cpp-ui when it wants to enable or disable scissoring to clip content.
	/// @param[in] enable True to enable scissoring, false to disable it.
	virtual void EnableScissorRegion(bool enable) = 0;
	/// Called by pp-cpp-ui when it wants to change the scissor region.
	...
	/// Called by pp-cpp-ui when it wants to enable or disable the clip mask.
	/// @param[in] enable True to enable the clip mask, false to disable it.
	virtual void EnableClipMask(bool enable);
	/// Called by pp-cpp-ui when it wants to set or modify the contents of the clip mask.
	...
	/// Called by pp-cpp-ui when it wants the renderer to use a new transform matrix.
	...
	/// Called by pp-cpp-ui when it wants to push a new layer onto the render stack, setting it as the new render target.
	...
	/// Called by pp-cpp-ui when it wants to pop the render layer stack, setting the new top layer as the render target.
	virtual void PopLayer();

	/// Called by pp-cpp-ui when it wants to store the current layer as a new texture to be rendered later with geometry.
	...
	/// Called by pp-cpp-ui when it wants to store the current layer as a mask image, to be applied later as a filter.
	...
	/// Called by pp-cpp-ui when it wants to compile a new filter.
	...
	/// Called by pp-cpp-ui when it no longer needs a previously compiled filter.
	/// @param[in] filter The handle to a previously compiled filter.
	virtual void ReleaseFilter(CompiledFilterHandle filter);

	/// Called by pp-cpp-ui when it wants to compile a new shader.
	...
	/// Called by pp-cpp-ui when it wants to render geometry using the given shader.
	...
	/// Called by pp-cpp-ui when it no longer needs a previously compiled shader.
	/// @param[in] shader The handle to a previously compiled shader.
