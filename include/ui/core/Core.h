...
/**
    pp-cpp-ui library core API.
 */

/// Initialises pp-cpp-ui.
UI_CORE_API bool Initialise();
/// Shutdown pp-cpp-ui.
UI_CORE_API void Shutdown();

/// Returns the version of this pp-cpp-ui library.
...
/// @lifetime The interface must be kept alive until after the call to ui::Shutdown.
UI_CORE_API void SetRenderInterface(RenderInterface* render_interface);
...
///           ui::Shutdown. Alternatively, the render interface can be destroyed after all contexts it belongs to have been
///           destroyed, and a subsequent call has been made to ui::ReleaseRenderManagers.
/// @return A non-owning pointer to the new context, or nullptr if the context could not be created.
UI_CORE_API Context* CreateContext(const String& name, Vector2i dimensions, RenderInterface* render_interface = nullptr,
...
UI_CORE_API bool RemoveContext(const String& name);
...
UI_CORE_API Context* GetContext(const String& name);
...
UI_CORE_API Context* GetContext(int index);
/// Returns the number of active contexts.
/// @return The total number of active pp-cpp-ui contexts.
UI_CORE_API int GetNumContexts();
...
UI_CORE_API bool LoadFontFace(const String& file_path, bool fallback_face = false, Style::FontWeight weight = Style::FontWeight::Auto,
	int face_index = 0);
...
/// @param[in] face_index The index of the font face within a font collection.
/// @return True if the face was loaded successfully, false otherwise.
/// @lifetime The pointed to 'data' must remain available until after the call to ui::Shutdown.
UI_CORE_API bool LoadFontFace(Span<const byte> data, const String& family, Style::FontStyle style,
	Style::FontWeight weight = Style::FontWeight::Auto, bool fallback_face = false, int face_index = 0);

/// Registers a generic pp-cpp-ui plugin.
UI_CORE_API void RegisterPlugin(Plugin* plugin);

/// Unregisters a generic pp-cpp-ui plugin.
UI_CORE_API void UnregisterPlugin(Plugin* plugin);
...
UI_CORE_API EventId RegisterEventType(const String& type, bool interruptible, bool bubbles,
...
UI_CORE_API StringList GetTextureSourceList();
/// Forces all texture handles loaded and generated by pp-cpp-ui to be released.
...
UI_CORE_API void ReleaseTextures(RenderInterface* render_interface = nullptr);
...
UI_CORE_API bool ReleaseTexture(const String& source, RenderInterface* render_interface = nullptr);
/// Forces all compiled geometry handles generated by pp-cpp-ui to be released.
...
UI_CORE_API void ReleaseCompiledGeometry(RenderInterface* render_interface = nullptr);
...
UI_CORE_API void ReleaseRenderManagers();
